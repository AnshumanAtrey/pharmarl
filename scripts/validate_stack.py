...
def check_standard_oracles() -> dict[str, float | None]:
    ...
    # Multi-target classifier trio — the held-out generalization test setup:
    # Train on DRD2 + GSK3B, hold out JNK3 for transfer evaluation.
    print("\n  Multi-target classifier trio (DRD2 train | GSK3B train | JNK3 held-out):")
    ...
    # DRD2 sanity: haloperidol (a known D2 antagonist drug) must score ~0.99.
    # Catches model-cache corruption before training burns Colab quota.
    print("\n  DRD2 cache sanity check:")
    if scores.get("DRD2") is not None:
        try:
            o = Oracle(name="DRD2")
            halo = float(o(haloperidol))
            verdict = "OK" if halo > 0.9 else "WEIRD — oracle may be corrupted"
            print(f"    [{verdict}]    haloperidol = {halo:.4f}  (expect > 0.9)")
        except Exception as e:
...
def main() -> int:
    ...
    header("Summary")
    composite_oracles = ("QED", "SA", "DRD2", "CYP3A4_Veith")
    multitarget_trio = ("DRD2", "GSK3B", "JNK3")
    composite_count = sum(1 for n in composite_oracles if oracle_scores.get(n) is not None)
    multitarget_count = sum(1 for n in multitarget_trio if oracle_scores.get(n) is not None)
    print(f"  Composite-reward oracles:    {composite_count}/4 (QED, SA, DRD2, CYP3A4)")
    print(f"  Held-out trio:               {multitarget_count}/3 (DRD2, GSK3B, JNK3)")
    print(f"  Stage 2 docking oracle:      {docking_name or 'not active (Stage 1 = DRD2 + GSK3B + JNK3)'}")
    print(f"  SELFIES round-trip:          {'OK' if selfies_ok else 'FAIL'}")
    print(f"  RDKit basics:                {'OK' if rdkit_ok else 'FAIL'}")

    stage1_ready = composite_count >= 3 and selfies_ok and rdkit_ok
    held_out_ready = multitarget_count == 3
    if not stage1_ready:
        print("\n  [RED] Stage 1 stack not ready. Fix imports/oracles before training.")
        return 2
    if not held_out_ready:
        print("\n  [YELLOW] Composite stack works but held-out trio incomplete.")
        print("           Single-target training will work; held-out generalization test will not.")
        return 1

    if docking_name:
        print("\n  [GREEN] Stage 1 + Stage 2 + held-out trio all working — full PharmaRL capabilities.")
        return 0
    enabled = os.environ.get("PHARMARL_ENABLE_DOCKING", "").lower() in ("1", "true", "yes")
    if enabled:
        print("\n  [YELLOW] Stage 2 docking probe enabled but failed (pyscreener/Vina/OpenBabel missing).")
        print("           Stage 1 multi-target (DRD2 + GSK3B + JNK3) still works.")
        return 1
    print("\n  [GREEN] Stage 1 multi-target stack ready. Held-out generalization test can run.")
    print("           Train on DRD2 + GSK3B; evaluate transfer on JNK3.")
